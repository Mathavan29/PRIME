...
import BrokerSidebar from './BrokerSidebar'; // Importing BrokerSidebar instead of Sidebar
import './BrokerDashboard.css'; // Renamed CSS file
...
const BrokerDashboard = () => {
  ...
  useEffect(() => {
    const fetchTotalHouses = async () => {
      try {
        const soldHousesResponse = await fetch('http://localhost:8080/api/houses/getall');
        const waitingHousesResponse = await fetch('http://localhost:8080/api/plans/getall');
